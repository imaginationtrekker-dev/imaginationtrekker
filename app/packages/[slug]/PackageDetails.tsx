...
export default function PackageDetails({ packageData }: PackageDetailsProps) {
  ...
  const [isMomentsLightboxOpen, setIsMomentsLightboxOpen] = useState(false);
  const [momentsLightboxIndex, setMomentsLightboxIndex] = useState(0);
  ...
  // Keyboard: Escape to close Moments lightbox, arrows to navigate
  useEffect(() => {
    if (!isMomentsLightboxOpen) return;
    const moments = packageData.moments_of_trek || [];
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape") setIsMomentsLightboxOpen(false);
      if (moments.length <= 1) return;
      if (e.key === "ArrowLeft")
        setMomentsLightboxIndex((prev) =>
  ...
  }, [isMomentsLightboxOpen, packageData.moments_of_trek]);
  ...
  return (
    <>
                    ...
                    <div className="package-moments-gallery">
                      {packageData.moments_of_trek.map((url, index) => (
                        <div
                          ...
                          tabIndex={0}
                          onClick={() => {
                            setMomentsLightboxIndex(index);
                            setIsMomentsLightboxOpen(true);
                          }}
                          onKeyDown={(e) => {
      ...
      {/* Moments of Trek Lightbox with Slider */}
      {isMomentsLightboxOpen &&
        packageData.moments_of_trek &&
        packageData.moments_of_trek.length > 0 && (
          <div
            className="package-lightbox-overlay"
            onClick={() => setIsMomentsLightboxOpen(false)}
          >
            <button
              className="package-lightbox-close"
              onClick={() => setIsMomentsLightboxOpen(false)}
            ...
            {packageData.moments_of_trek.length > 1 && (
              <>
                <button
                  className="package-lightbox-nav package-lightbox-nav-prev"
                  onClick={(e) => {
                ...
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    setMomentsLightboxIndex((prev) =>
                      prev >= packageData.moments_of_trek!.length - 1
                        ? 0
                        : prev + 1,
                    );
                  }}
            ...
            >
              <Image
                src={packageData.moments_of_trek[momentsLightboxIndex]}
                alt={`Moment ${momentsLightboxIndex + 1}`}
                width={1200}
                height={800}
                className="package-lightbox-image"
                style={{
            ...
            {packageData.moments_of_trek.length > 1 && (
              <div
                className="package-lightbox-dots"
                onClick={(e) => e.stopPropagation()}
              >
                {packageData.moments_of_trek.map((_, index) => (
                  <button
                    key={index}
                    type="button"
                    className={`package-lightbox-dot ${
                      momentsLightboxIndex === index ? "active" : ""
                    }`}
                    onClick={(e) => {
                      e.stopPropagation();
                      setMomentsLightboxIndex(index);
