...
export default function PdfEnquiriesPage() {
  const supabase = createClient();
  const [enquiries, setEnquiries] = useState<PdfEnquiry[]>([]);
  ...
  const loadEnquiries = async () => {
    try {
      ...
      const response = await fetch(`/api/pdf-enquiries?${params.toString()}`);

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || "Failed to load PDF enquiries");
      }
      ...
      setError(err instanceof Error ? err.message : "Failed to load PDF enquiries");
      console.error("Error loading PDF enquiries:", err);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="dashboard_page enquiries-page" style={{ margin: "24px" }}>
      <div className="heading_block">
        ...
        >
          <div>
            <h3>PDF Enquiries</h3>
            <p>View all successful PDF download requests</p>
          </div>
          <button
      ...
      <div className="enquiries-content-wrap">
        <div className="enquiries-filters">
          <div className="enquiries-search-wrap">
            ...
            <input
              type="text"
              placeholder="Search by name, WhatsApp, email, package..."
              value={searchQuery}
        ...
        {!loading && enquiries.length === 0 && (
          ...
          >
            {searchQuery
              ? "No PDF enquiries found matching your search."
