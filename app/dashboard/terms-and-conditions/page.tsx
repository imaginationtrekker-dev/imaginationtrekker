...
export default function TermsAndConditionsPage() {
  ...
  const loadTerms = async () => {
    try {
      ...
      if (sessionError || !session) {
        throw new Error('You must be logged in to view terms and conditions. Please refresh the page.');
      }
      ...
      if (error) {
        ...
        if (error.message.includes('relation') || error.message.includes('does not exist')) {
          throw new Error('Terms and Conditions table does not exist. Please run the terms_and_conditions_table.sql script in Supabase.');
        }
        throw error;
      }
      setTerms(data || []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to save terms and conditions');
  ...
  const handleEdit = (term: TermsAndConditions) => {
    ...
    setFormData({
      ...
      main_content: term.main_content,
    });
    setIsModalOpen(true);
  };
  ...
  return (
    <div className='dashboard_page'>
      <div className='heading_block'>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <p>Manage terms and conditions content for your website.</p>
          </div>
          <button
      ...
      {loading ? (
        <div style={{ padding: '40px', textAlign: 'center', color: '#6b7280' }}>Loading terms and conditions...</div>
      ) : terms.length === 0 ? (
        <div style={{ padding: '40px', textAlign: 'center', color: '#6b7280' }}>
          No terms and conditions found. Click "Add Terms & Conditions" to create one.
        </div>
      ) : (
              ...
              <tbody>
                {terms.map((term) => (
                  <tr key={term.id} style={{ borderBottom: '1px solid #e5e7eb' }}>
                    <td style={{ padding: '12px 16px', fontSize: '14px', color: '#1f2937', fontWeight: 500 }}>{term.main_title}</td>
                    <td style={{ padding: '12px 16px', fontSize: '14px', color: '#1f2937' }}>{term.subtitle || '-'}</td>
                    <td style={{ padding: '12px 16px', fontSize: '14px', color: '#1f2937', maxWidth: '400px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {term.main_content.replace(/<[^>]*>/g, '').substring(0, 100)}...
                    </td>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h3 style={{ fontSize: '20px', fontWeight: 600, color: '#1f2937', margin: 0 }}>
                {editingTerm ? 'Edit Terms and Conditions' : 'Add New Terms and Conditions'}
              </h3>
              <button
                onClick={handleCloseModal}
